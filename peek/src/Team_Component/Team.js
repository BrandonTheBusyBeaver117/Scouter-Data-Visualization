...
export class Team extends Component {
    ...
    */
	...
	getImages() {
		...
		return axios
			.get(`https://res.cloudinary.com/drzeip1bi/image/list/${teamNumber}.json`)
			.then((response) => {
				const imageLinks = [];
	...
	render() {
		...
		// Testing to make sure readings are accurate

		let numFail = 0;
		let numSuccess = 0;

		if (positiveData.length > 0 && negativeData.length > 0) {
			positiveData.forEach((matchResult) => (numSuccess += matchResult.toUpperCase() === "TRUE" ? 1 : 0));
			negativeData.forEach((matchResult) => (numFail += matchResult.toUpperCase() === "TRUE" ? 1 : 0));
		} else if (positiveData.length > 0 && negativeData.length === 0) {
			positiveData.forEach((matchResult) => (matchResult.toUpperCase() === "TRUE" ? numSuccess++ : numFail++));
		} else if (negativeData.length > 0) {
			negativeData.forEach((matchResult) => (matchResult.toUpperCase() === "FALSE" ? numSuccess++ : numFail++));
		}
		...
		return (
			...
			>
				...
				<p>{numSuccess / (numFail + numSuccess)}</p>
