...
export default function InformationSource(props) {
    ...
    const handleUpdate = (changedEventKey, changedSheetLink, changedFile, propsCurrentEventKey, propsCurrentInputSource) => {
    ...
    const createForm = typeInputSource => {
        if (typeInputSource === "csv") {
            ...
            return <form>
                ...
                <input type = "text" value = {sheetLink} placeholder = "Google sheet link" onChange = {(event) => setSheetLink(event.target.value)}/>
            </form>
        }
    }
    ...
    return <div id = "InformationSource">
        ...
        <input type = "text" placeholder = "Input your event key" value = {eventKey} onChange = {(event) => setEventKey(event.target.value)}/>
        ...
        <div id = "typeChooser">
            <button onClick = {() => setSourceTypeSelected("link")} style = {buttonHighlighting( sourceTypeSelected, "link")}>Google Sheets Link</button>
            <button onClick = {() => setSourceTypeSelected("csv")} style = {buttonHighlighting( sourceTypeSelected, "csv")}>Csv File</button>
        </div>
