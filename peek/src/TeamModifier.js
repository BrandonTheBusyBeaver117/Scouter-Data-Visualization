...
export class TeamModifier extends Component {
	...
	sortTeamsQualities(quality) {
		...
		for (const teamKey of arrayOfChosenTeams) {
			...
			// Might want to change "" to the default config or whatever
	...
	render() {
		...
		let teamComponents = this.renderTeamComponents(
			this.state.chosenTeams,
			this.state.googleSheetHeaders,
			this.state.sortedTeamInformation,
